...
function l1Plan() {
  ...
  return shell('1F 概念平面圖', `PROJECT / MODEL ${versionSlug} · SITE-XY · 0.5 m 最小讀圖格 · ACTIVE ${active.id}`, `${grid()}${northArrow()}
    ...
    <line x1="${planX(0)}" y1="${planY(0)}" x2="${planX(0.5)}" y2="${planY(0)}" class="wallLine"/><line x1="${planX(2.5)}" y1="${planY(0)}" x2="${planX(39)}" y2="${planY(0)}" class="wallLine" data-entity="F-L1-Y0-01"/><text x="${planX(15)}" y="${planY(0) + 34}" text-anchor="middle" class="tiny">L1 Y0 自然灰清水模外牆（EN-01 玻璃入口除外）</text>
    <rect x="60" y="845" width="1530" height="82" rx="9" class="note"/><text x="82" y="875" class="small">操場男廁＝1 WC＋2 小便斗＋2 洗手槽；操場女廁＝2 WC＋2 洗手槽。既有器具保留；L1 Y0 外牆改採自然灰清水模並保留 EN-01 玻璃入口。</text><text x="82" y="902" class="small">四間廁所入口維持 1.00 m 無門板且不設遮擋版；ST-01 維持 X20.5～X29／Y0.5～Y2.0、雙連續鋼箱梯梁與梯下開放。</text>`, 'DRAW-L1-PLAN');
}
...
function l2Plan() {
  ...
  return shell('2F 概念平面圖', `PROJECT / MODEL ${versionSlug} · Review A · 固定正交樓板 +3.30 m · SITE-XY`, `${northArrow()}${rect(active.site, 'reference', '1F 基地參照')}${rect(plate, 'outline')}
    ...
    ${grid()}
    <line x1="${planX(29)}" y1="${planY(0)}" x2="${planX(41)}" y2="${planY(0)}" class="glassLine"/><text x="${planX(36.5)}" y="${planY(0) + 34}" text-anchor="middle" class="tiny">Y0 大面安全玻璃</text>
...
function l3Plan() {
  ...
  return shell('3F 概念平面圖', `PROJECT / MODEL ${versionSlug} · L3 +6.88 m · +${active.l3.planRotation.toFixed(1)}°／+${active.l3.mirror.leanFromVertical.toFixed(1)}° 工作值 · SITE-XY`, `${northArrow()}<g transform="translate(184.8 118.8) scale(0.78)" data-drawing-scale="fit-complete-rotated-roof">${rect(active.site, 'reference', '1F 基地參照')}${rect(active.l2.floorPlate, 'reference', '固定 L2 投影')}
    ...
    <g data-entity="${roof.entityId}" data-area="${roof.area}" data-complete-roof="true" transform="${rotationTransform}"><rect x="${planX(roof.bounds.x1)}" y="${planY(roof.bounds.y2)}" width="${(roof.bounds.x2-roof.bounds.x1)*scale}" height="${(roof.bounds.y2-roof.bounds.y1)*scale}" class="roof"/></g>
    <g data-entity="${pv.entityId}" data-area="${pv.area}" data-roof-area="${pv.roofArea}" data-coverage-percent="${pv.coveragePercent}" data-perimeter-setback="${pv.perimeterSetback}" data-capacity-status="${pv.capacityStatus}" transform="${rotationTransform}"><rect x="${planX(pv.bounds.x1)}" y="${planY(pv.bounds.y2)}" width="${(pv.bounds.x2-pv.bounds.x1)*scale}" height="${(pv.bounds.y2-pv.bounds.y1)*scale}" class="pv"/>${pvColumns.map((x) => `<line x1="${planX(x)}" y1="${planY(pv.bounds.y1)}" x2="${planX(x)}" y2="${planY(pv.bounds.y2)}" class="pvGrid"/>`).join('')}${pvRows.map((y) => `<line x1="${planX(pv.bounds.x1)}" y1="${planY(y)}" x2="${planX(pv.bounds.x2)}" y2="${planY(y)}" class="pvGrid"/>`).join('')}<text x="${planX((pv.bounds.x1+pv.bounds.x2)/2)}" y="${planY((pv.bounds.y1+pv.bounds.y2)/2)+4}" text-anchor="middle" class="small" style="fill:#fff;font-weight:700">屋頂光電高覆蓋率排布 ${pv.area.toFixed(1)} m²／${pv.coveragePercent.toFixed(2)}%</text></g>
    ${grid()}
    <text x="${planX(34)}" y="${planY(13.4)}" text-anchor="middle" class="tiny">完整 3F 屋頂；太陽能板保留周邊 ${pv.perimeterSetback.toFixed(2)} m，模組／容量／發電量待定</text></g>
    <rect x="60" y="845" width="1610" height="82" rx="9" class="note"/><text x="82" y="875" class="small">3F 旋轉量體上方新增完整屋頂，並補滿外傾鏡牆兩端與直立側牆的三角收邊；SITE-XY 0.5 m 格線置於主要填色上方保持可讀。</text><text x="82" y="902" class="small">為容納旋轉外挑，L3 圖面以完整屋頂範圍縮放；太陽能板周邊保留 ${pv.perimeterSetback.toFixed(2)} m，概念覆蓋 ${pv.area.toFixed(1)}／${pv.roofArea.toFixed(1)} m²（${pv.coveragePercent.toFixed(2)}%）。</text>`, 'DRAW-L3-PLAN');
}
...
function sectionDrawing() {
  ...
  const body = `${sectionGrid(sx, sz)}<g><line x1="${sx(0)}" y1="${sz(0)}" x2="${sx(41)}" y2="${sz(0)}" class="outline"/>${[0.1,0.3,3.3,6.88,10.48].map((z) => `<line x1="${sx(0)}" y1="${sz(z)}" x2="${sx(43.5)}" y2="${sz(z)}" class="level"/><text x="${sx(43.7)}" y="${sz(z)+4}" class="tiny">+${z.toFixed(2)} m</text>`).join('')}</g>
    ...
    <rect x="60" y="842" width="1600" height="84" rx="9" class="note"/><text x="82" y="873" class="small">剖面採 X／Z 同尺度，新增 0.5 m 次格線、2.5 m 主格線與 X／Z 數值標籤。L2 天花與 L3 屋頂均為連續板；L3 屋頂接到外傾鏡牆上緣。</text><text x="82" y="902" class="small">3F 太陽能板於周邊保留 ${pv.perimeterSetback.toFixed(2)} m 後盡量鋪滿完整屋頂；容量、檢修道、結構、防水、眩光、消防與儲能仍須專業核定。</text>`;
