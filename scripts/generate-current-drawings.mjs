...
function l1Plan() {
  ...
  return shell('1F 概念平面圖', `PROJECT / MODEL ${versionSlug} · SITE-XY · 0.5 m 最小讀圖格 · ACTIVE ${active.id}`, `${grid()}${northArrow()}
    ...
    <rect x="60" y="845" width="1450" height="82" rx="9" class="note"/><text x="82" y="875" class="small">操場男廁＝1 WC＋2 小便斗＋2 洗手槽；操場女廁＝2 WC＋2 洗手槽。0.6.3 新增器具均緊鄰原有器具配置，原有器具保留。</text><text x="82" y="902" class="small">四間廁所入口維持 1.00 m 無門板且不設遮擋版；ST-01 維持 X20.5～X29／Y0.5～Y2.0、雙連續鋼箱梯梁與梯下開放。</text>`, 'DRAW-L1-PLAN');
}
...
function l2Plan() {
  ...
  return shell('2F 概念平面圖', `PROJECT / MODEL ${versionSlug} · Review A · 固定正交樓板 +3.30 m · SITE-XY`, `${northArrow()}${rect(active.site, 'reference', '1F 基地參照')}${rect(plate, 'outline')}
    ...
    <line x1="${planX(32.5)}" y1="${planY(2.5)}" x2="${planX(41)}" y2="${planY(2.5)}" class="wallLine"/><text x="${planX(36.8)}" y="${planY(2.5) - 8}" text-anchor="middle" class="tiny">Y2.5 清水模分隔牆</text>
...
function l3Plan() {
  ...
  return shell('3F 概念平面圖', `PROJECT / MODEL ${versionSlug} · L3 +6.88 m · +${active.l3.planRotation.toFixed(1)}°／+${active.l3.mirror.leanFromVertical.toFixed(1)}° 工作值 · SITE-XY`, `${grid()}${northArrow()}${rect(active.site, 'reference', '1F 基地參照')}${rect(active.l2.floorPlate, 'reference', '固定 L2 投影')}
    ...
    <g data-entity="${pv.entityId}" data-area="${pv.area}" data-capacity-status="${pv.capacityStatus}">${rect(pv, 'pv')}${[33.25,34,34.75].map((x) => `<line x1="${planX(x)}" y1="${planY(pv.bounds.y1)}" x2="${planX(x)}" y2="${planY(pv.bounds.y2)}" class="pvGrid"/>`).join('')}${[9.125,10.25,11.375].map((y) => `<line x1="${planX(pv.bounds.x1)}" y1="${planY(y)}" x2="${planX(pv.bounds.x2)}" y2="${planY(y)}" class="pvGrid"/>`).join('')}<text x="${planX(34)}" y="${planY(10.25) + 4}" text-anchor="middle" class="tiny" style="fill:#fff;font-weight:700">太陽能預留 ${pv.area.toFixed(1)} m²</text></g>
    <text x="${planX(34)}" y="${planY(12.15)}" text-anchor="middle" class="tiny">模組／容量／發電量待定</text>
    <rect x="60" y="845" width="1580" height="82" rx="9" class="note"/><text x="82" y="875" class="small">3F 目前維持低使用密度的高位設備／維修空間；教師觀察、泳姿分析與環境教育不設固定房間，保留未來彈性。乾式維修儲藏僅列考慮。</text><text x="82" y="902" class="small">屋頂預留 ${pv.area.toFixed(1)} m² 輕量太陽能區並預留電力／EMS 介面；儲能優先設於地面層獨立戶外機櫃，3F 一般室內不放電池。容量、確切位置、結構與消防仍待專業驗證。</text>`, 'DRAW-L3-PLAN');
}
...
function sectionDrawing() {
  ...
  const body = `<g><line x1="${sx(0)}" y1="${sz(0)}" x2="${sx(41)}" y2="${sz(0)}" class="outline"/>${[0.1,0.3,3.3,6.88].map((z) => `<line x1="${sx(0)}" y1="${sz(z)}" x2="${sx(43.5)}" y2="${sz(z)}" class="level"/><text x="${sx(43.7)}" y="${sz(z)+4}" class="tiny">+${z.toFixed(2)} m</text>`).join('')}</g>
    ...
    <g data-entity="${pv.entityId}" data-area="${pv.area}" data-capacity-status="${pv.capacityStatus}"><rect x="${sx(pv.bounds.x1)}" y="${sz(pv.baseElevation)}" width="${(pv.bounds.x2-pv.bounds.x1)*36}" height="10" class="pv"/><line x1="${sx(pv.bounds.x1)}" y1="${sz(pv.baseElevation)+10}" x2="${sx(pv.bounds.x1)}" y2="${sz(6.88)}" class="structure"/><line x1="${sx(pv.bounds.x2)}" y1="${sz(pv.baseElevation)+10}" x2="${sx(pv.bounds.x2)}" y2="${sz(6.88)}" class="structure"/><text x="${sx((pv.bounds.x1+pv.bounds.x2)/2)}" y="${sz(pv.baseElevation)-10}" text-anchor="middle" class="tiny">3F 屋頂太陽能預留 ${pv.area.toFixed(1)} m²／容量待定</text></g>
    ...
    <rect x="60" y="842" width="1540" height="84" rx="9" class="note"/><text x="82" y="873" class="small">剖面採 X／Z 同尺度。ST-01 與 ST-02 均採懸空薄踏步、封閉踢面與雙連續鋼箱梯梁語彙；ST-02 梯下只放低矮可移除植栽，不封閉、不設深土槽或水景。</text><text x="82" y="902" class="small">3F 太陽能區為固定支承線上的概念預留，不代表模組或容量已定；儲能優先落地於獨立戶外機櫃，不在 3F 一般室內畫電池。結構、消防、機電與維修動線仍須專業核定。</text>`;
