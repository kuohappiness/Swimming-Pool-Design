...
- `reference/generated/viewer-model.json`：由 active `GEO-0.6.4` 產生。
- `reference/generated/concept-content.json`：由公開理念 Markdown 與 `{{active:...}}` token 產生。

兩者必須有相同 `modelVersion=0.6.4` 與 `modelHash`。viewer model 另須包含 `activeGeometryRevisionId=GEO-0.6.4`、`coordinateSystemId=SITE-XY` 及每個 bounded entity 的 canonical `entityBounds`。hash、token、scene ID 或有限幾何不符時直接失敗，不顯示 fallback 幾何。
...
- L1 Y0 外牆除 `EN-01` 玻璃入口外，須連續表達自然灰清水模；四間廁所立面必須依 active `toiletEntrances` 切出 1.00 m 無門板洞口，不得以實心牆或門扇遮蔽。
...
- L2 顯示 Review A 的 L 形面池走道、X29 觀景窗、X32 男女 1.00 m 無門片入口、無座懸空站立桌／飲水機／盆栽，以及男女各 15 間含隔間 1.20 × 1.20 m 淋浴模組、1 WC 與 2 洗手槽。Y0 外牆須為 X29～X41 全寬安全玻璃；Y2.5 分隔牆須由 X32 連續封至 X41、不得留樓梯間通往更衣室的開口；L2 天花板須完整覆蓋 X29～X41／Y0～Y13.5。
...
WebGL 不可用或 `?forceFallback=1` 時提供靜態總覽、五場景內容與最新 v0.6.4 圖集連結。
