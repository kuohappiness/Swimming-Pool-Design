...
- `reference/generated/viewer-model.json`：由 active `GEO-0.6.5` 產生。
- `reference/generated/concept-content.json`：由公開理念 Markdown 與 `{{active:...}}` token 產生。

兩者必須有相同 `modelVersion=0.6.5` 與 `modelHash`。viewer model 另須包含 `activeGeometryRevisionId=GEO-0.6.5`、`coordinateSystemId=SITE-XY` 及每個 bounded entity 的 canonical `entityBounds`。hash、token、scene ID 或有限幾何不符時直接失敗，不顯示 fallback 幾何。
...
- L2 顯示 Review A 的 L 形面池走道、X29 觀景窗、X32 男女 1.00 m 無門片入口、無座懸空站立桌／飲水機／盆栽，以及男女各 15 間含隔間 1.20 × 1.20 m 淋浴模組、1 WC 與 2 洗手槽。Y0 外牆須為 X29～X41 全寬安全玻璃，採雙面淡藍透明材質、邊框、高光與豎梃，從 Y0／Y14 都清楚可辨；Y2.5 清水模牆是後方獨立平面，須由 X32 連續封至 X41、不得留樓梯間通往更衣室的開口。L2 天花板須完整覆蓋 X29～X41／Y0～Y13.5。
...
圖層固定為 `site`、`l1`、`water`、`l2`、`l3`、`roof`、`circulation`、`rain`、`energy`、`annotations`。桌機與觸控支援 orbit、pan、zoom、一般固定視角、圖層切換與構件選取；canvas 與等價控制須可鍵盤操作。390 × 844 不得水平溢出。
...
WebGL 不可用或 `?forceFallback=1` 時提供靜態總覽、五場景內容與最新 v0.6.5 圖集連結。
