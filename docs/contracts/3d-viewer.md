...
- `reference/generated/viewer-model.json`：由 active `GEO-0.6.2` 產生。
- `reference/generated/concept-content.json`：由公開理念 Markdown 與 `{{active:...}}` token 產生。

兩者必須有相同 `modelVersion=0.6.2` 與 `modelHash`。viewer model 另須包含 `activeGeometryRevisionId=GEO-0.6.2`、`coordinateSystemId=SITE-XY` 及每個 bounded entity 的 canonical `entityBounds`。hash、token、scene ID 或有限幾何不符時直接失敗，不顯示 fallback 幾何。

`model/analysis-registry.json` 的 solar hash 不符時顯示 `stale`；只有完成 v0.6.2 solar 重算與既有回歸測試後才可更新為 `current`。
...
- 廁所內裝須可辨識洗手台與具有門板的 WC 隔間；四個入口不設遮擋版，可直接面向洗手台。男廁洗手台貼 Y0、女廁洗手台貼 Y7.5，所有 WC 隔間貼 Y3.5；泳池男廁其中一座小便斗位於 X31 且避開入口。
- L2／L3 基準板為 12 × 13.5 m；L2 +3.30 m、L3 +6.88 m。
- L2 男女各顯示 15 間淨 1.00 × 1.00 m 淋浴間。`ST-02` 由 X32.5 起步、固定在 Y0.5～Y2.0、朝 +X 上升至有頂的 3F 正交到達翼，不得以斜向樓梯或戶外景觀區作唯一通路。
...
WebGL 不可用或 `?forceFallback=1` 時提供靜態總覽、五場景內容與最新 v0.6.2 圖集連結。
