...
- `REF-001`：`SRC-SITE-001` 原始衛星底圖、MODEL 0.6.6、ACTIVE GEO-0.6.6、SITE-XY 與北向。
- `V066-L1`：內嵌 `DRAW-L1-PLAN-v0.6.6.svg`。
- `V066-L2`：內嵌 `DRAW-L2-PLAN-v0.6.6.svg`。
- `V066-L3`：內嵌 `DRAW-L3-PLAN-v0.6.6.svg`。
- `V066-SECTION`：內嵌 `DRAW-LONGITUDINAL-SECTION-v0.6.6.svg`。
...
每張圖須保留 `v0.6.6`、`GEO-0.6.6`、`SITE-XY` 與「非施工圖」聲明。SVG 由 `npm run drawings:v066` 重現，PNG 必須由同一 SVG render 產生。現行 HTML 必須內嵌 SVG，不得以 PNG `<image>` 取代；`V066-L3` 另提供獨立、預設勾選的「太陽能板」控制，取消時隱藏整個 `RF-PV-RES-01` group。

## 驗收

- model sheet registry 恰為 `REF-001`、`V066-L1`、`V066-L2`、`V066-L3`、`V066-SECTION`。
- 四張 SVG／PNG 均存在，SVG metadata 與 active model 一致。
- Viewer 與 solar-study 最新圖面連結都指向 `#V066-L1`。
