...
公開 shell 顯示 `MODEL 0.9.3` 與 `ACTIVE GEO-0.8.2`；V067 SVG／PNG 本體仍是不可變的 `v0.6.7`／`GEO-0.6.7` 完稿，不因展示層發布改寫。0.9.1 的 ORIGIN 與 3D 資訊精修、0.9.2 的文字與留白調整、0.9.3 的手機目錄滑動提示與滿版顯示都不改圖冊內容。
