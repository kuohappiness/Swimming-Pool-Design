...
| TASK-005 | 完整驗證與本機 release commit | in_progress | 0.2.0 | [發布門檻](06_WORKFLOW_AND_RELEASES.md) | TASK-001～TASK-004、TASK-006 | 文件、模型、測試、build、視覺 smoke 與 diff 檢查通過；建立 release 記錄與本機 `release: v0.2.0` commit；不 tag、不 push |

`TASK-004` 已完成 0.2.0 版號同步；`TASK-005` 正在執行最終發布門檻與本機 release commit。
