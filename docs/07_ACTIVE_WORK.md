...
0.6.5 依 2026-07-24 使用者核准內容，修正 L1 分段立面與西端退縮、兩處透明玻璃屋簷、L2 Y0 玻璃視覺辨識、L3 圖面旋轉／透明光電圖層、縱剖遺漏牆面及 3D 泳池深度剖視。既有日照分析輸入不變，不重新執行日照最佳化。

| ID | 工作 | 狀態 | 目標版本 | Owner／規格 | 依賴 | 完成條件 |
| --- | --- | --- | --- | --- | --- | --- |
| TASK-050 | 同步實作 0.6.5 模型、圖面、Viewer、HTML、文件與發布 | done | 0.6.5 | [DEC-107～DEC-113](04_DECISIONS_AND_OPEN_ITEMS.md)、OPEN-011／OPEN-014／OPEN-016／OPEN-022 | TASK-049 | canonical model、L1 平面與 Viewer 統一表達 X0.5 西端牆、X1～3 `EN-01`、X0.5～31 玻璃池廳及 X31～39 清水模服務本體；X0～0.5 設玻璃屋簷／雨水回收，X31～39／Y13.5～14.5 補透明玻璃屋頂並標示突出建築邊線。L2 Y0 全玻璃在 Y0／Y14 視角都可辨識，後方 Y2.5 清水模牆不變。L3 SVG 共用單一 +25.5° transform，淡藍透明光電圖層在內嵌 SVG HTML 可獨立勾選。縱剖補 X0.5、X29、X41 牆及投影入口。Viewer 新增 Y0 長邊固定泳池剖視、可稍向下旋轉、斜底／水深標示與離開後完整恢復。solar inputHash 維持 current 且不跑最佳化；完整 build、桌面／行動 smoke、圖面視覺檢查及 `git diff --check` 通過後 commit 並 push `main`。 |
...
## 15. 0.7.0／0.8.0 整合執行順序

`TASK-053` 已完成。其餘工作依下列階段執行；同一階段內的任務可在隔離工作區平行進行，各任務仍以「依賴」欄為實際啟動門檻。0.7.0 負責 Walkthrough、碰撞與游泳核心，0.8.0 只建立 enhanced rendering；正式發布順序固定為 0.7.0 → 0.8.0。

| 階段 | 0.7.0 工作線 | 0.8.0／共用工作線 | 啟動門檻 |
| --- | --- | --- | --- |
| 0｜已完成 | `TASK-053` | — | `TASK-052` |
| 1｜共同門檻 | — | `TASK-059` | `TASK-053` |
| 2｜平行啟動 | `TASK-054` | `TASK-060` | `TASK-059` |
| 3｜核心實作 | `TASK-055` | `TASK-061`、`TASK-062` | 各自完成前項依賴 |
| 4｜水域核心 | `TASK-056` | — | `TASK-055` |
| 5｜功能整合 | `TASK-057` | `TASK-063` | `TASK-056` 及各自前項依賴 |
| 6｜驗證門檻 | `TASK-058`（發布 0.7.0） | `TASK-064` | 各自完成前項依賴 |
| 7｜最終整合 | — | `TASK-065`（整合並發布 0.8.0） | `TASK-058`、`TASK-064` |

下表是 Active Work 的正式狀態 owner，並依上述執行階段排列：

| ID | 工作 | 狀態 | 目標版本 | Owner／規格 | 依賴 | 完成條件 |
| --- | --- | --- | --- | --- | --- | --- |
| TASK-053 | 建立 Walkthrough 唯讀資料邊界、型別、設定與模組骨架 | done | 0.7.0 | [DEC-117／DEC-118](04_DECISIONS_AND_OPEN_ITEMS.md)、[核准設計](archive/specs/2026-07-24-3d-walkthrough-mvp-design.md)、[implementation plan](archive/specs/2026-07-24-3d-walkthrough-mvp-implementation-plan.md)、[子專案索引](subprojects/3d-walkthrough/README.md) | TASK-052 | 已建立 `reference/src/3d-viewer/walkthrough/` public interface、deep-readonly／deep-freeze adapter、獨立體驗 config、13 個必要 entity、8 個 walk surfaces、2 座 stair descriptors、池殼／water volume、7 個 openings、6 個 safe spawns 與 capability flags。10 組 identity／hash／SITE-XY／adapter／entity／bounds／finite geometry／stair-pool 破壞性輸入均 fail closed；4 項 focused tests、44 項全套測試、typecheck、完整 build 與 `git diff --check` 通過。canonical source 建置前後 SHA-256 均為 `0ADC3DD2EA845C694C7475EF4587BABBBE73C8352FB326200BDC2DA58B1F6AAD`，`project-model.json` 無差異且沒有 walkthrough 體驗欄位。 |
| TASK-059 | 建立共用渲染接縫與平行工作流 validator | done | 0.8.0 shared baseline | [DEC-119／DEC-120](04_DECISIONS_AND_OPEN_ITEMS.md)、[視覺擬真設計](archive/specs/2026-07-24-3d-visual-fidelity-design.md)、[implementation plan](archive/specs/2026-07-24-3d-visual-fidelity-implementation-plan.md) | TASK-053 | 已建立 `MaterialRegistry`、`EnvironmentEffect`、`FrameEffectPipeline`、`VisualAssetAdapter` 與 `RenderQualityProfile` 窄介面及 idempotent baseline implementation；`scene-factory.ts` 不再直接建立材質，`main.ts` 不再直接設定環境或呼叫 renderer。docs validator 可接受核准且隔離的 0.7／0.8 各一項 `in_progress`，並以破壞性 fixtures 拒絕同版本、未知版本、缺少 DEC／spec 核准及共用 integration task 衝突。13 項 focused tests、48 項全套測試、docs／reference validation、typecheck、production build 與 `git diff --check` 通過；desktop／390 × 844 實機檢查涵蓋五場景、selection、剖視、registry/runtime 標記及無水平溢出，重新載入無新增 warning／error。 |
| TASK-054 | 整合 Inspect／Walkthrough camera mode 與桌機／手機輸入 | done | 0.7.0 | [核准設計](archive/specs/2026-07-24-3d-walkthrough-mvp-design.md)、[implementation plan](archive/specs/2026-07-24-3d-walkthrough-mvp-implementation-plan.md) | TASK-053、TASK-059 | 已建立可重入 `CameraModeManager`、固定步進 loop、可替換 `PlayerController` strategy、desktop Pointer Lock＋drag fallback 與 touch 左移動／右環視 adapters；selection 可 suspend／resume，Inspect camera／target／FOV、場景、圖層、剖視、panel scroll、selection 與 focus 可逆保存。20 次進出與 listener balance、30／60 FPS 固定步進、Pointer Lock 拒絕、blur／visibility／pointercancel／orientation reset 均通過；desktop 實機完整還原雨天＋剖視＋關閉 energy layer＋L2 玻璃 selection。390 × 844 自我檢查修正漫遊 grid 壓縮後，viewport 全寬、雙控制區各半、退出 target 44 px 且無水平溢出；fallback 隱藏漫遊入口。12 項 focused tests、typecheck、docs 與 `git diff --check` 通過。 |
| TASK-060 | 建立 0.8.0 視覺基準、資產清單、授權與效能預算 | done | 0.8.0 | [視覺擬真設計](archive/specs/2026-07-24-3d-visual-fidelity-design.md)、[implementation plan](archive/specs/2026-07-24-3d-visual-fidelity-implementation-plan.md)、[子專案索引](subprojects/3d-visual-fidelity/README.md) | TASK-059 | 已保存 desktop 1440 × 900／mobile 390 × 844 五場景、池側／校側與兩種 walkthrough 人眼尺度共 14 張 hash 鎖定 PNG；baseline report 記錄 12-frame SwiftShader 相對 frame-time、183,484-byte Viewer 傳輸量、canvas 尺寸、input／rendering runtime 與零外部 origin，並明示不可當硬體 FPS 驗收。5 項 project-authored CC0 本地資產均登錄 ID、用途、path、source、author、license、SHA-256、byte size、tier、required／optional 與 loading priority；high／medium／low 預算固定為 24／12／6 MiB，low 關閉 shadow／AO／enhanced water／postprocess。自我檢查發現並修正並行 headless RAF 節流造成的假性數據；4 項 focused tests、55 項全套測試、typecheck、完整 production build、視覺抽查與 `git diff --check` 通過，package／modelVersion 仍為 0.6.7。 |
| TASK-055 | 實作 capsule collision、重力、樓梯代理、安全出生點與全區域移動 | done | 0.7.0 | [核准設計](archive/specs/2026-07-24-3d-walkthrough-mvp-design.md)、[implementation plan](archive/specs/2026-07-24-3d-walkthrough-mvp-implementation-plan.md) | TASK-053、TASK-054 | 已建立由 live `siteRoot.matrixWorld` 烘焙的 surface／solid／pool-bottom／stair proxies、capsule penetration resolution、重力、ground snap、連續雙跑坡面與 `WalkMovement`；六個 semantic safe spawns 均驗證 clear＋supported，teleport／NaN／掉落 recovery 一次清速度。自我檢查修正入口前場缺少 2 m apron support、`l2-arrival` 錯停在 +3.11 m 梯跑、L1 perimeter 被錯拉成跨樓層牆三項問題。4 項 collision tests 驗證 EN-01、solid facade、樓梯側阻擋、ST-01 雙向、ST-02 上行、307°／Y 反向／rotated L3 visual root 一致及 30／60 FPS 路徑等價；60 項全套測試、typecheck、完整 production build、desktop／390 × 844 實機與完整 e2e 通過。實機由入口穿行後自動踏上 +0.300 m 池畔，返回安全點、44 px 行動按鈕、Inspect restore、fallback 與零 console warning／error 均正常。 |
| TASK-061 | 實作 PBR 材質系統與不改碰撞的建築表面細節 | done | 0.8.0 | [視覺擬真設計](archive/specs/2026-07-24-3d-visual-fidelity-design.md)、OPEN-016 | TASK-060 | 已建立 async `EnhancedTextureLoader`、`EnhancedPbrMaterialRegistry` 與 `EnhancedSurfaceDetailAdapter`，以 8 個本地 CC0 base-color／normal／roughness 資產提供公尺 repeat、sRGB／NoColorSpace 通道、共享 texture／material instance、high／medium／low anisotropy 與 optional water fallback。清水模、玻璃、鋼、池磚、池畔、欄杆、鏡面、光電與水面均具 semantic PBR；edges 只以 `visualOnly`／`collisionExcluded` disposable lines 表達接縫、框料與壓頂，不新增門、欄杆、結構或通行語意。4 項 material tests 加上 8 項 walkthrough data／collision tests驗證 required failure、optional degrade、tier restore、bounds／selectables／canonical source 不變；typecheck、docs 與 `git diff --check` 通過。`?rendering=enhanced` 診斷入口以實際 Viewer scene graph 成功載入 registry／adapter；總覽與池側人工抽查可辨識玻璃框、池磚、池畔、鋼與光電，無 shader、z-fighting、console warning／error；正式預設仍保留 baseline。 |
| TASK-062 | 實作 HDR／色調映射、陰影、環境與可降級後製 | done | 0.8.0 | [視覺擬真設計](archive/specs/2026-07-24-3d-visual-fidelity-design.md)、[implementation plan](archive/specs/2026-07-24-3d-visual-fidelity-implementation-plan.md) | TASK-060 | 已建立本地 courtyard sky `PMREMGenerator`、四組既有 environment profiles、ACES／sRGB tone mapping、per-scene exposure、stable PCF shadow bias／normalBias 及 `EnhancedFrameEffectPipeline`；保留 `RenderPass → SSAOPass → OutputPass` 可選 pipeline，medium／low 直接 render 且 low 關閉 shadow／environment map。TASK-064 實測半解析度 SSAO 在 1440 × 900 SwiftShader 僅 1.35 FPS，正式 high 因此改採 PBR／PMREM／ACES／PCF shadow 直接渲染。所有 render targets 支援 resize／dispose／context restore，optional environment 失敗保留色彩／fog／燈光並提供 diagnostic。自我檢查修正缺少 OutputPass 導致無 console error 的白屏、day 過曝、top-level await production incompatibility，並將 enhanced code 動態分割以降低 baseline 首屏成本。5 項 environment tests、12 項 material／walkthrough tests、68 項全套測試、typecheck、docs、reference validation、最終 production bundle 與 `git diff --check` 通過。high／low、總覽／向雨／向光與 low walkthrough 實機檢查皆可操作、無水平溢位或 console warning／error；quality 不改 scene ID、相機、圖層、selection、collision 或 movement。正式預設仍保留 baseline。 |
| TASK-056 | 實作泳池 water volume、水面／水下游泳、上岸與水下環境 | done | 0.7.0 | [核准設計](archive/specs/2026-07-24-3d-walkthrough-mvp-design.md)、[implementation plan](archive/specs/2026-07-24-3d-walkthrough-mvp-implementation-plan.md) | TASK-055 | 可見水面、water volume 與池底碰撞共用 active `POOL-01` 衍生狀態；X3 1.20 m→X28 1.50 m 斜底、固定步進三軸游泳、水線 hysteresis、浮力／下潛／上浮、池壁／池底、clearance 上岸及安全返回池畔皆由自動測試鎖定。水下霧化／色調／音訊 hook 支援 low quality 與 reduced motion，音訊失敗不阻斷移動；觸控上浮／下潛可持續按住。17 項 walkthrough focused tests 與 typecheck 通過。 |
| TASK-057 | 完成全區域跳轉、HUD、可用性、效能階級與未來擴充介面 | done | 0.7.0 | [核准設計](archive/specs/2026-07-24-3d-walkthrough-mvp-design.md)、[implementation plan](archive/specs/2026-07-24-3d-walkthrough-mvp-implementation-plan.md) | TASK-054、TASK-055、TASK-056 | 六區選擇器以 `SafeSpawnAreaRegistry` 只跳至 clear＋supported semantic spawn；桌機／390 × 844 HUD、返回、退出、44 px touch targets、safe-area、ARIA live notice 與 focus restore 實機通過且無水平溢出。`AdaptiveFrameTimeMonitor` 以兩個持續超標觀測窗與單向 high→medium→low hysteresis 降級 pixel ratio、shadow、水下效果與 camera motion，不改 simulation；自我檢查修正首次 shader compile 誤降級。Input、Movement、VisualAsset、Area、Environment 均維持窄介面；pointer lock／audio／optional visual failure 與 WebGL fallback 只降級功能。25 項整合 focused tests、typecheck、production build 與桌機／手機實機通過。 |
| TASK-063 | 提升水面、玻璃、基地環境與視覺資產層 | done | 0.8.0 | [視覺擬真設計](archive/specs/2026-07-24-3d-visual-fidelity-design.md)、[implementation plan](archive/specs/2026-07-24-3d-visual-fidelity-implementation-plan.md) | TASK-056、TASK-061、TASK-062 | `EnhancedWaterPresentation` 只接受 scene-factory／0.7 movement 傳入的同一 water surface elevation，第二份水面狀態會 fail closed；normal、透射、水線、水下與 cutaway state 可依 tier 降級。`EnhancedVisualAssetAdapter` 依 `SITE-01` visual bounds 放置 project-authored 程序化植栽、人物尺度與設備，全部 `visualOnly`／`collisionExcluded`，low 可移除昂貴視覺，optional failure 只回到 baseline，dispose 會移除 group 並釋放 geometry／material。2 項專屬測試、25 項整合 focused tests、typecheck、production build、enhanced Inspect／Walkthrough／入水與 390 × 844 實機通過。 |
| TASK-058 | 完成 Walkthrough 回歸、桌機／手機 E2E、契約、版本與 0.7.0 發布驗證 | done | 0.7.0 | [核准設計](archive/specs/2026-07-24-3d-walkthrough-mvp-design.md)、[implementation plan](archive/specs/2026-07-24-3d-walkthrough-mvp-implementation-plan.md)、[3D Viewer 契約](contracts/3d-viewer.md)、[Release 0.7.0](releases/0.7.0.md) | TASK-053、TASK-054、TASK-055、TASK-056、TASK-057、TASK-059 | package／model／active revision／generated data 已同步 0.7.0；model diff 只含 release metadata，V067 幾何、solar inputHash 與 professional approvals 不變。78 項完整測試、69 份 Markdown、reference validation、typecheck、production build 與 `git diff --check` 通過。desktop E2E 走查六區、入口、入水、水下、返回並恢復雨天＋energy 關閉＋`POOL-01` selection＋剖視 snapshot；390 × 844 以持續 touch input 入水／下潛、驗證 44 px controls、退出與零水平溢出；fallback、solar-study、atlas 與零 browser error 全數通過。自我檢查修正 dataset 首幀 race、select focus、underwater effect race 與 synthetic pointer capture 失敗。兩份規格 completed 並封存；依使用者指示，commit／push 延後至 0.8.0 全部完成後一次執行。 |
