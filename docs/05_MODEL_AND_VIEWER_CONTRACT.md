...
- 以同一太陽位置函式依日期、時間、經緯度與 UTC offset 計算高度角及方位角，不保存另一份冬夏角度表。
- 左側平面固定 1F，只旋轉 2F 更衣量體；右側剖面只調整面池端牆外傾。
- 讓季節、時間、2F 水平旋轉及牆面外傾四個控制共同更新反射結果。
- 清楚區分「操作預設」與「正式設計值」，並將結構支點、最終角度與性能連結 `OPEN-011`。
- 支援 320 px 以上手機、鍵盤操作、可讀 SVG 標題／描述及無外部執行依賴。
...
13. 校名、經緯度、時區與北向只由模型提供；互動頁不得另存第二套。
14. 冬至 09:00 操作預設能導向泳池，夏至 09:00 同一預設不命中泳池。
15. 反射函式對方位環繞、背光、平面偏差與向下角度具有單元測試。
16. build 同時輸出 `dist/reference/index.html` 與 `dist/reference/solar-study/index.html`。
