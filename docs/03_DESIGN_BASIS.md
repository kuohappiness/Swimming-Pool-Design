...
本文件是目前設計數值與空間意圖的權威來源。`model/project-model.json` 的 `activeGeometryRevisionId=GEO-0.8.2` 是唯一現行機器可讀幾何；圖面、分析與 Viewer 均以 `resolveActiveGeometry()` 解析，不由檔名、日期或最大版號猜測最新版。

狀態語意：

- `confirmed`：使用者已確認的概念輸入；不等於現況測量或專業簽證。
- `working`：v0.6.7 現行協調值，可供圖面與模型一致表達。
...
| 本地 +X 真北方位 | 307°；由泳池遠端指向服務核心 | confirmed canonical |
| 本地 −X 真北方位 | 127°；同一長軸反方向／面池基準 | derived |
...
| L2 Y0 Viewer 表達 | 與其他安全玻璃外牆共用同一材質系統、反射高光、上下框與豎梃；Y0／Y14 視角均須清楚辨識前方玻璃，Y2.5 清水模牆維持後方獨立平面 | confirmed output intent |
| Viewer 構件選取 | 保留畫布點選、鍵盤輪選、下拉選單與右側資訊面板；3D 畫布不顯示黃色或其他外接選取框 | confirmed interaction intent |
...
分析使用 `SRC-SITE-003` 的 8,760 小時 PVGIS TMY，方法修訂版為 `SOLAR-METHOD-1.0.0`；active solar contract 明列等效鏡面反射率 `ρ=0.75`、玻璃太陽透射率 `τ=0.60`、日照時段 07:00～17:00、方位容差 28°與最低向下角 8°。目標為「暖季池面新增嚴格為零，再最大化冷季池面新增」。
...
0.5.0 的 +26.5°／+3.1°套入現行池體後會造成暖季 +200.722 kWh，故已淘汰，不得靜默沿用。0.6.7 只修正 Viewer L2 `splitAxisY` 資料投影、錯置牆面與玻璃材質回復，未改變池體、鏡牆角度、旋轉主矩形、支點、固定屋頂接收面或其他日照分析輸入，因此沿用 2026-07-23 已驗證工作值，不重新執行完整最佳化。0.9.6 audit correction 依 `DEC-142` 把既有門檻、能量假設與方法修訂版正式移入 active solar contract，並以擴充後的 solar `inputHash` 重新驗證；數值輸入與上述結果未變。新增屋簷明確是非接收面雨棚，完整 L3 屋頂與高覆蓋率太陽能排布仍未納入遮蔭、上部接收、眩光或發電分析。以上仍是概念階段能量篩檢，不是 lux、眩光、水溫、設備節能、光電發電或施工性能保證；`OPEN-011`／`OPEN-022` 繼續管理材料、構造與專業光熱驗證。

## 7. 現行輸出

- 空間圖集只顯示 `REF-001`、`V067-L1`、`V067-L2`、`V067-L3`、`V067-SECTION`。
- v0.6.7 SVG／PNG 由 `npm run drawings:v067` 從 active geometry 重現；HTML 直接內嵌 SVG，L3 另有預設勾選的「太陽能板」控制。
...
v0.6.7 已完成概念層級結構—空間—設備整合檢查，但建築、結構、機電、消防與無障礙的 `professionalApprovals` 全為 `false`。未決事項包含 `OPEN-008`、`OPEN-009`、`OPEN-011`、`OPEN-014`、`OPEN-016`～`OPEN-022`；本成果不是施工、發包、結構計算或法規簽證文件。
