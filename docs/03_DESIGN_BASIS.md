...
本文件是目前設計數值與空間意圖的權威來源。`model/project-model.json` 的 `activeGeometryRevisionId=GEO-0.6.0` 是唯一現行機器可讀幾何；圖面、分析與 Viewer 均以 `resolveActiveGeometry()` 解析，不由檔名、日期或最大版號猜測最新版。

狀態語意：

- `confirmed`：使用者已確認的概念輸入；不等於現況測量或專業簽證。
- `working`：v0.6.0 現行協調值，可供圖面與模型一致表達。
...
| --- | --- | --- |
| 模型版本 | `0.6.0` | active |
| Active revision | `GEO-0.6.0` | active |
...
四間廁所彼此不得互通；泳池組與操場組不可共用穿越式空間。器具數是概念程序，隔間厚度、管道間、隱私牆、門淨寬、無障礙廁所與法定數量仍由 `OPEN-008`／`OPEN-009` 驗證。
...
0.5.0 的 +26.5°／+3.1°套入 v0.6.0 池體後會造成暖季 +200.722 kWh，故已淘汰，不得靜默沿用。以上仍是概念階段能量篩檢，不是 lux、眩光、水溫、設備節能或施工性能保證；`OPEN-011` 繼續管理材料、構造與專業光熱驗證。

## 7. 現行輸出

- 空間圖集只顯示 `REF-001`、`V060-L1`、`V060-L2`、`V060-L3`、`V060-SECTION`。
- v0.6.0 SVG／PNG 由 `npm run drawings:v060` 從 active geometry 重現。
- 3D Viewer、日照 HTML、圖面與圖集皆顯示 model version、active revision、SITE-XY 或 model hash。
- v0.5.0 舊圖可留在歷史資料夾，但不得出現在現行 HTML current tab。

## 8. 專業驗證邊界

v0.6.0 已完成概念層級結構—空間—設備整合檢查，但建築、結構、機電、消防與無障礙的 `professionalApprovals` 全為 `false`。未決事項集中於 `OPEN-008`、`OPEN-009`、`OPEN-011`、`OPEN-014`、`OPEN-016`、`OPEN-017`、`OPEN-018`；本成果不是施工、發包、結構計算或法規簽證文件。
