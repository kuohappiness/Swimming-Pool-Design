...
9. 基礎參數變更時，以 clone 修改該參數再驗證所有衍生幾何同步更新，避免 renderer 保存結果常數。
10. `deferred` 欄位必須連結 OPEN ID；在決策關閉前不得加入數值 fallback。
...
公開首頁是 `https://kuohappiness.github.io/Swimming-Pool-Design/`，日照研究位於 `/solar-study/`。`.github/workflows/deploy-pages.yml` 在每次 `main` push 或人工觸發時執行：

- `npm ci` 依 lockfile 安裝依賴，再以 `npm run build` 驗證並建置。
- 只有 `dist/reference/` 會成為 Pages artifact；`dist/` 不納入 Git 歷史。
- build 成功後才部署至 `github-pages` environment；失敗時保留上一個成功網站。
- 未來圖集、日照研究或其他多頁入口只要由同一模型／renderer 建置並 push 至 `main`，就會自動同步。
...
- 可變參數測試證明 L2、樓梯與屋頂使用同一推導結果。
- deferred 幾何在資料與圖面均清楚可辨，沒有偽裝成已定案的數值。
